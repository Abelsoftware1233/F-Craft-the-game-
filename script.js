...
function createGame(levelIdx) {
  ...
  return {
    ...
    coins: map.coins.map(c => ({ x: c.x*TILE+8, y: c.y*TILE+8, w:14, h:14, collected:false, anim:0 })),
...
function tryMine(player, mapData) {
  ...
  const tx = Math.floor((cx + dir*TILE) / TILE);
  const ty = Math.floor(cy / TILE);

  // Check adjacent tiles
  const candidates = [
    [tx, ty], [tx, ty-1], [tx, ty+1],
    [Math.floor(cx/TILE), ty-1]
  ];
  let mined = false;
    ...
    if (MINABLE.has(tile)) {
      ...
      if (game.mineProgress >= needed) {
        ...
        if (drop) {
          ...
          // Particles
          spawnParticles(
            canvas.getBoundingClientRect().left + bx*TILE - game.cam.x + 16,
            canvas.getBoundingClientRect().top  + by*TILE - game.cam.y + 16,
        ...
        game.mineBlock=null; game.mineProgress=0;
        HUD.update(getHUDState());
        Crafting.render && Crafting.render(game.inventory);
      }
      mined=true; break;
    }
  }
  if (!mined) { game.mineBlock=null; game.mineProgress=0; }
}
  ...
  game.enemies.forEach(e => {
    ...
    if (p.x+p.w > e.x && p.x < e.x+e.w && p.y+p.h > e.y && p.y < e.y+e.h) {
      ...
      if (p.vy > 0 && p.y+p.h < e.y+e.h/2+8) {
        ...
        HUD.update(getHUDState());
        spawnParticles(
          canvas.getBoundingClientRect().left + e.x - game.cam.x,
          canvas.getBoundingClientRect().top  + e.y - game.cam.y,
          '#ff6600', 6
        );
      } else {
        // Take damage
...
function checkCollectibles() {
  ...
  game.coins.forEach(c => {
    ...
    if (p.x+p.w>c.x && p.x<c.x+c.w && p.y+p.h>c.y && p.y<c.y+c.h) {
      c.collected=true; game.coins+=1; game.score+=100;
      HUD.update(getHUDState());
    }
  });
...
function getHUDState() {
  return { lives:game.lives, score:game.score, coins:game.coins,
...
function update(dt) {
  ...
  game.coins.forEach(c => { if(!c.collected) c.anim=(c.anim||0)+0.1; });
